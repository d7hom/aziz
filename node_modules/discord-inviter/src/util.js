...
class inviteTracker extends Event {
  ...
  async ready() {
    this.client.guilds.cache.map(async (g) => {
      try {
        var invites = await g.invites.fetch();
  ...
  async inviteUpdate(invite) {
    try {
      var invites = await invite.guild.invites.fetch();
  ...
  async memberAdd(member) {
    ...
    try {
        ...
        if (findInvite) {
          ...
          if (findInvite?.uses < vanityData?.uses) {
            ...
            resData = {
              ...
              inviter: await member.guild.fetchOwner(),
              member: member,
            };
          }
        }
      ...
      if (!resData) {
        var Owner = await member.guild.fetchOwner();
        resData = {
          invite: {
            code: "Unknown",
            url: "Unknown",
            count: guildInvites
              ?.filter((i) => i?.inviter?.id == Owner?.id)
              ?.reduce((p, v) => p + v.uses, 0),
          },
          inviter: Owner,
          member: member,
